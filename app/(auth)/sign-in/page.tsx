...
export default function SignIn() {
  ...
  const [email, setEmail] = useState('ailsoncgt@gmail.com');
  const [password, setPassword] = useState('fejepe10');
  ...
  if (loadingEmail || loadingGithub || loadingGoogle) { 
    return(
  ...
  if (errorEmail || errorGithub || errorGoogle) {
    return <div>Error: {JSON.stringify(errorEmail)}</div>
  }

  if (user) {
    router.push('/dashboard')
    return <p className='font-bold'>Welcome back, {user.displayName}!</p>
  }
          ...
          <form>
            <div className="grid gap-6">
              <div className="flex flex-col gap-4">
                <Button variant="outline" className="w-full" onClick={() => signinGithub()}>
              ...
              <div className="grid gap-6">
                ...
                <Button type="submit" className="w-full" onClick={() => signinEmail()}>
                  Login
                </Button>
              </div>  
