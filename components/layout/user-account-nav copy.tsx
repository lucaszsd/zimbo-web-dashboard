...
import { LayoutDashboard, LogOut, Settings, Smile } from "lucide-react";
          ...
          <Drawer.Content className="fixed inset-x-0 bottom-0 z-50 mt-24 overflow-hidden rounded-t-[10px] border bg-background px-3 text-sm">
            ...
            <div className="flex items-center justify-start gap-2 p-2">
              <div className="flex flex-col">
                {/* {user.disp && <p className="font-medium">{user.name}</p>}
                {user.email && (
  ...
  return (
    <DropdownMenu open={open} onOpenChange={setOpen}>
      ...
      <DropdownMenuContent align="end">
        ...
        <DropdownMenuItem asChild>
          <Link href="/dashboard/feedbacks" className="flex items-center space-x-2.5">
            <Smile className="size-4" />
            <p className="text-sm">Feedbacks</p>
          </Link>
        </DropdownMenuItem>
